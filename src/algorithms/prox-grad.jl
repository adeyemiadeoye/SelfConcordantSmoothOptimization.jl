...
Base.@kwdef mutable struct ProxGradient <: ProximalMethod
    ...
    label::String = "Proximal-Gradient"
end
init!(method::ProxGradient, x) = method
function step!(method::ProxGradient, reg_name, model, hμ, As, x, x_prev, ys, iter)
    λgr = model.λ .* hμ.grad(x)
    Hr_diag = hμ.hess(x)
    ...
    prox_m = invoke_prox(model, reg_name, x + step_size*d, one.(Hr_diag), model.λ, step_size)
