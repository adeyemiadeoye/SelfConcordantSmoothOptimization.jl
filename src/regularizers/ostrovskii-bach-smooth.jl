...
OsBaSmootherL1L2(mu::IntOrFloat; val=x->osba_smooth_l1.(x;μ=mu), grad=(Cmat,x)->osba_smooth_grad_l1.(x;μ=mu), hess=(Cmat,x)->osba_smooth_hess_l1.(x;μ=mu)) = OsBaSmootherL1L2(mu, osba_smooth_Mh, osba_smooth_ν, val, grad, hess)
